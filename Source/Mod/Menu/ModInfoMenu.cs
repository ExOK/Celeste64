	...
	private void InitItems()
	{
		...
		Add(new Toggle(
			...
			() =>
			{
				...
				{
					Save.GetOrMakeMod(Mod.ModInfo.Id).Enabled = !Save.GetOrMakeMod(Mod.ModInfo.Id).Enabled;

					if (Save.GetOrMakeMod(Mod.ModInfo.Id).Enabled)
					{
					...
					else
					{
						if (Mod.DisableSafe(true)) // If it is not safe to disable the mod
						{
							...
							{
								Save.GetOrMakeMod(Mod.ModInfo.Id).Enabled = true; // Override the toggle if the operation can't be done.
						...
						if (Mod.GetDependents().Count > 0)
						{
							...
							{
								Save.GetOrMakeMod(Mod.ModInfo.Id).Enabled = true; // Override the toggle if the operation was cancelled.
			...
			() => Mod != null ? Save.GetOrMakeMod(Mod.ModInfo.Id).Enabled : false)
		);
